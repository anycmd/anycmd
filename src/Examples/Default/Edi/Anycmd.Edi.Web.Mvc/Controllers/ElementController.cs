...
namespace Anycmd.Edi.Web.Mvc.Controllers
{
    ...
    public class ElementController : AnycmdController
    {
        ...
        public ActionResult Tooltip(Guid? elementId)
        {
            if (elementId.HasValue)
            {
                return this.PartialView(
                    "Partials/Tooltip",
                    Host.NodeHost.Ontologies.GetElement(elementId.Value).Element);
            }
            else
            {
        ...
        public ActionResult TooltipEdit(string isInner, string tooltip, Guid? elementId)
        {
            if (elementId.HasValue)
            {
                var element = Host.NodeHost.Ontologies.GetElement(elementId.Value);
                if (Request.HttpMethod == "POST")
                {
                    var entity = Host.NodeHost.Ontologies.GetElement(elementId.Value).Element;
                    if (entity != null)
                    {
                        Host.UpdateElement(
        ...
        public ActionResult GetPlistElements(GetPlistElements input)
        {
            ...
            if (!Host.NodeHost.Ontologies.TryGetOntology(input.OntologyId.Value, out ontology))
            {
        ...
        public ActionResult GetNodeElementCares(GetNodeElementCares input)
        {
            ...
            var nodeElementCares = Host.NodeHost.Nodes.GetNodeElementCares(node);
        ...
        public ActionResult AddOrUpdateElementActions()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    ElementDescriptor element;
                    Host.NodeHost.Ontologies.TryGetElement(inputModel.ElementId, out element);
        ...
        {
            return this.HandleSeparateGuidString(Host.RemoveElement, id, ',');
        }
        #endregion
    }
}
